...
const SVG_MAP_BINDINGS = {
  ...
  hamstrings: 'hamstrings', gluteal: 'gluteal', calves: 'calves', forearms: 'forearms'
};

const MUSCLE_LABELS = { chest: 'Chest', lats: 'Lats', deltoids: 'Shoulders', biceps: 'Biceps', triceps: 'Triceps', trapezius: 'Traps', abs: 'Core', obliques: 'Core', quadriceps: 'Quads', hamstrings: 'Hamstrings', gluteal: 'Glutes', calves: 'Calves', forearms: 'Forearms' };
...
function renderMuscleMap() {
  ...
  Object.entries(freq).sort((a,b)=>b[1]-a[1]).forEach(([id, count]) => {
    ...
    if (lbl && !shown.has(lbl)) { 
      ...
      if (svgContainer) {
        const svgTargetId = SVG_MAP_BINDINGS[id] || id;
        const targets = svgContainer.querySelectorAll(`[id*="${svgTargetId}" i], [class*="${svgTargetId}" i]`);
        targets.forEach(el => {
          el.style.fill = color;
